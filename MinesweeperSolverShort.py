#!/usr/bin/python3
# /usr/bin/python3 "/Users/rajtiller/Source/Minesweeper/MinesweeperSolverShort.py"
import random
...
def makeChoice(columna, rowa):
    ...
    while row < 1 or row > length:
        try:
            row = int(input("Which Row? (1-" + str(length) + ")" + "\n" + printBoard()))
        except:
            row = 0
    flagMaybe = ""
    while flagMaybe != "" and flagMaybe != "r" and flagMaybe != "f":
        flagMaybe = input(
            "Reveal (r or leave blank) or flag(f)? (1-"
            + str(length)
            + ")"
            + "\n"
            + printBoard()
...
def playGame():
    ...
    sqrev()
    print("Og board:\n" + printBoard())
    input("Press enter to solve board")
    print(
        "Squares revealed: "
        + str(squaresRevealed)
        + "/"
    ...
    while (
        squaresRevealed > -1
        and squaresRevealed < length * length - (length * length) // 8
    ):
        calcValid()
        if len(safeSquares) != 0:
            for square in safeSquares:
                makeChoice((square - 1) % length + 1, (square - 1) // (length) + 1)
                print("board:\n" + printBoard())
                print(
                    "Revealed ("
                    + str((square - 1) % length + 1)
                    + ","
                    + str((square - 1) // length + 1)
                    + ")"
                )
            ...
            squaresRevealed = -1
            print(rbold("This position is unsolvable"))
    revealBoard()
...
def calcValidBoards():
    ...
    for _ in range(2 ** len(aroundRevealed)):
        ...
        if boardsDrawn % 100000 == 0:
            print(
                "Boards drawn: "
                + str((1000 * boardsDrawn) // 2 ** len(aroundRevealed) / 10)
                + "%"
            )
        # print(printBoardSimple(newBoard))
